...
// There are two main differences between dynamic typing and static typing that you should be
// aware of when writing transformation scripts.

// First, dynamically-typed languages perform type checking at runtime, while statically 
//typed languages perform type checking at compile time. This means that scripts written in
// dynamically-typed languages (like Groovy) can compile even if they contain errors that
// will prevent the script from running properly (if at all). If a script written in a 
//statically-typed language (such as Java) contains errors, it will fail to compile until
