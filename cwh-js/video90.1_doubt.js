...
A closure is a feature of the JavaScript language that allows a function to remember and access variables and functions defined outside of its own
 scope, even after the outer function has returned or the variables have gone out of scope. In other words, a closure is a function that has access
  to variables in its parent function's scope, even when the parent function has finished executing.

To create a closure in JavaScript, you define a function inside another function and return the inner function. The inner function has access to 
the variables and functions in the outer function's scope, even after the outer function has returned.
...
let closure = outer();
closure(); // output: 5
In this example, the outer function defines a variable a and a nested function inner that logs the value of a to the console. The outer function 
then returns the inner function, which is assigned to the variable closure.

When we call closure(), the inner function executes and logs the value of a, which is 5. Even though the outer function has already returned and
 its variables are no longer in scope, the inner function still has access to the value of a through closure.

Closures are commonly used in JavaScript for various programming patterns, such as the module pattern, where you use a closure to create private
 variables and methods, or for handling asynchronous code with callbacks.
...
When outer() is called and the inner() function is defined, the inner() function creates a closure over the variable a. This means that the 
inner() function has access to the value of a even after outer() has finished executing and the variable a would normally be out of scope.

When inner() is returned and assigned to the variable closure, closure becomes a closure that retains access to the value of a. When closure() is 
called, it executes the inner() function and logs the value of a, which is still accessible due to the closure.
...
You are correct that the inner() function is called when outer() is called in this example. However, the important thing to understand is that the
 inner() function is not executed until it is invoked, even if it is defined inside another function.

When outer() is called, it defines the a variable and the inner() function, but it does not execute the inner() function. Instead, it returns the 
inner() function, which is assigned to the variable closure. At this point, the inner() function is not executed yet.

When you later call closure() by invoking closure(), the inner() function is executed and it logs the value of a, which is still accessible due to 
the closure.

So even though the inner() function is defined inside the outer() function, it is not executed until it is invoked. And when it is invoked, it has
 access to the variables defined in the outer function's scope, even if the outer function has already returned.
...
In the example you provided, the inner() function is not invoked directly when outer() is called. Instead, the inner() function is returned from 
the outer() function and assigned to the variable closure.
